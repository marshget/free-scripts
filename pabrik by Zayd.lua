...
function pnb()
    ...
    for _, pos in ipairs(posRange) do
        ...
        while not disconnected(status) and not scanTrash() and not needGrind 
          and inventory:getItemCount(block_id) > 1 
          and (world:getTile(x, y).fg == 0 or world:getTile(x, y).fg ~= block_id) do
    ...
    -- ====== BREAK setelah semua PUT selesai ======
    for _, pos in ipairs(posRange) do
        local x, y = pos[1], pos[2]
        while not disconnected(status) and not scanTrash() and not needGrind 
          and inventory:getItemCount(block_id) > 1 
...
function pnbv2()
    ...
    local myName = getBot().name
    local cfg = Bot_List[myName]
    local x, y = cfg.pos_break_x, cfg.pos_break_y - 1
    
    while not disconnected(status) and not scanTrash() and not needGrind and inventory:getItemCount(block_id) > 1 and (world:getTile(x, y).fg == 0 or world:getTile(x - 1, y).fg == 0 or world:getTile(x + 1, y).fg == 0)  do
        bot:place(x - 1, y, block_id)
        sleep(delay_place - 50)
        bot:place(x, y, block_id)
        sleep(delay_place - 50)
        bot:place(x + 1, y, block_id)
        randomSleep(delay_place - 30, delay_place)
    end
    takeAllItem()
    while not disconnected(status) and not scanTrash() and not needGrind and inventory:getItemCount(block_id) > 1 and (world:getTile(x, y).fg == block_id or world:getTile(x - 1, y).fg == block_id or world:getTile(x + 1, y).fg == block_id) do
        bot:hit(x - 1, y)
        sleep(delay_punch - 30)
        bot:hit(x, y)
        sleep(delay_punch - 30)
        bot:hit(x + 1, y)
        randomSleep(delay_punch - 50, delay_punch)
    end
    takeAllItem()
end
