...
function pnb()
    ...
    if not cfg then
        print("[PNB] Config untuk bot " .. myName .. " tidak ditemukan!")
        return
    end

    local posBreakX, posBreakY = cfg.pos_break_x, cfg.pos_break_y
    local rangePNB = cfg.rangePNB or 3
    ...
    print("[PNB] Bot:", myName, "Range:", rangePNB, "Pos:", posBreakX, posBreakY)

    -- ====== PLACE ======
    for _, pos in ipairs(posRange) do
        local x, y = pos[1], pos[2]
        print("[PNB] Place target:", x, y)
        ...
        while not disconnected(status)
          ...
          and world:getTile(x, y).fg == 0 do
            ...
            if inventory:getItemCount(block_id) <= 0 then
                print("[PNB] Habis block saat place.")
                break
            end
        end
    end
    ...
    for _, pos in ipairs(posRange) do
        ...
        while not disconnected(status)
          ...
          and world:getTile(x, y).fg == block_id do
            ...
            if inventory:getItemCount(block_id) <= 0 then
                print("[PNB] Habis block saat break.")
                break
            end
        end
    ...
    print("[PNB] Selesai untuk bot:", myName)
end
